const { app, BrowserWindow, dialog, ipcMain, globalShortcut, Menu, clipboard, session } = require('electron')
...
app.whenReady()
  ...
  .then(() => {
    ...
    EduNuageExam.mainWindow.webContents.on('dom-ready', function () {
      ...
      if (currentURL.hostname === 'docs.google.com' && currentURL.pathname.startsWith('/document/d/')) {
        currentCloud = 'google';
      }
      ...
      if (currentCloud) {
        ...
        if (currentCloud === 'google') {
          EduNuageExam.mainWindow.webContents.executeJavaScript(`
          const style = document.createElement('style');
          style.innerHTML = '#docs-tools-menu { display: none !important; }'; //goog-menuitem-disabled
        ...
        dialog.showMessageBox(EduNuageExam.mainWindow, {
          ...
          if (result.response === 0) { // Démarrer l'examen
            ...
            // On empêche l'utilisateur de changer de page en cliquant sur un lien
            EduNuageExam.mainWindow.webContents.on('will-navigate', (e, url) => {
              const event = {
                type: 'triedOpenUrl',
                time: new Date().getTime(),
                data: url
            ...
            // On empêche l'utilisateur d'ouvrir des liens qui s'ouvrent dans un nouvel onglet
            EduNuageExam.mainWindow.webContents.setWindowOpenHandler(({ url }) => {
              if (currentCloud === 'EN') {
